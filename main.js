	...
	Game.Init=function()
	{
		...
		window.onbeforeunload=function(event)
		{
			if (Game.prefs && Game.prefs.warn)
			{
				...
				if (event) event.returnValue=loc("Are you sure you want to close CClick?");
			}
		}
		...
		{
			Game.Notify(loc("Back up your save!"),loc("Hello again! Just a reminder that you may want to back up your CClick save every once in a while, just in case.<br>To do so, go to Options and hit \"Export save\" or \"Save to file\"!")+'<div class="line"></div><a style="float:right;" onclick="Game.prefs.showBackupWarning=0;==CLOSETHIS()==">'+loc("Don't show this again")+'</a>',[25,7]);
		}
		...
		if (!App)
		{
			Game.attachTooltip(l('httpsSwitch'),'<div style="padding:8px;width:350px;text-align:center;font-size:11px;">'+loc("You are currently playing CClick on the <b>%1</b> protocol.<br>The <b>%2</b> version uses a different save slot than this one.<br>Click this lock to reload the page and switch to the <b>%2</b> version!",[(Game.https?'HTTPS':'HTTP'),(Game.https?'HTTP':'HTTPS')])+'</div>','this');
			...
			Game.attachTooltip(l('topbarDiscord'),'<div style="padding:8px;width:250px;text-align:center;">Our official discord server.<br>You can share tips and questions about CClick and all our other games!</div>','this');
			Game.attachTooltip(l('topbarPatreon'),'<div style="padding:8px;width:250px;text-align:center;">Support us on Patreon and help us keep updating CClick!<br>There\'s neat rewards for patrons too!</div>','this');
			Game.attachTooltip(l('topbarMerch'),'<div style="padding:8px;width:250px;text-align:center;">CClick shirts, hoodies and stickers!</div>','this');
			Game.attachTooltip(l('topbarMobileCC'),'<div style="padding:8px;width:250px;text-align:center;">Play CClick on your phone!<br>(Android only; iOS version will be released later)</div>','this');
			Game.attachTooltip(l('topbarSteamCC'),'<div style="padding:8px;width:250px;text-align:center;">Get CClick on Steam!<br>Featuring music by C418.</div>','this');
		...
		{
			//for when Cclick won't load and you need your save
			console.log('===================================================');
			...
			if (Game.TickerN%2==0 || loreProgress>14)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					else
					{
						...
						'News : local man "done with CClick", finds the constant self-references "grating and on-the-nose".',
				...
				else
				{
					...
					choose([
						...
						'News : nation cheers as legislators finally outlaw '+choose(['cookie criticism','playing other games than CClick','pineapple on pizza','lack of cheerfulness','mosquitoes','broccoli','the human spleen','bad weather','clickbait','dabbing','the internet','memes','millennials'])+'!',
		...
		Game.NewUpgradeCookie({name:'Birthday cookie',desc:'<q>-</q>',icon:[22,13],power:years,price:99999999999999999999999999999});Game.last.baseDesc=loc("Cookie production multiplier <b>+%1%</b> for every year CClick has existed (currently: <b>+%2%</b>).",[1,Beautify(years)])+'<q>Thank you for playing CClick!<br>-Orteil</q>';
		...
		order=1800;Game.TieredAchievement('cclick forever and forever a hundred years cclick, all day long forever, forever a hundred times, over and over cclick adventures dot com','','Time machine',7);
		...
		order=1475;Game.TieredAchievement('The Prestige','<q>(Unrelated to the CClick feature of the same name.)</q>','Wizard tower',9);
		...
		order=36000;
		new Game.Achievement('So much to do so much to see',loc("Manage a cookie legacy for <b>at least a year</b>.")+'<q>Thank you so much for playing CClick!</q>',[23,11]);Game.last.pool='shadow';
		...
		Game.CpsAchievement('Speed\'s the name of the game (no it\'s not it\'s called CClick)');
		...
		order=500000;
		new Game.Achievement('CClick',loc("Unlock the final building."),[30,6]);
		Game.last.descFunc=function(){
		...
		if (!Game.OnAscend)
		{
			...
			if (Game.drawT%5==0)
			{
				...
				for (var i in Game.Objects)
				{
					...
					var classes='product';
					var price=me.bulkPrice;
					if (Game.cookiesEarned>=me.basePrice || me.bought>0) {classes+=' unlocked';lastLocked=0;me.locked=0;if (me.id==19){Game.Win('CClick');}} else {classes+=' locked';lastLocked++;me.locked=1;}
...
window.onload=function()
{
	if (!Game.ready)
	{
		var loadLangAndLaunch=function(lang,firstLaunch)
		{
			...
			//LoadLang('../CClick Localization/EN.js',function(lang){return function(){
